...
MAIN_FUNCTION
{
    ...
    // Initialize chain parameters early to avoid crashes in static initialization
    try {
        // Default to mainnet if no arguments are provided
        std::string chainName = "main";
        // Check if -testnet, -regtest, or -signet is specified
        for (int i = 1; i < argc; i++) {
            if (strcmp(argv[i], "-testnet") == 0 || strcmp(argv[i], "-testnet=1") == 0) {
                chainName = "test";
                break;
            } else if (strcmp(argv[i], "-regtest") == 0 || strcmp(argv[i], "-regtest=1") == 0) {
                chainName = "regtest";
                break;
            } else if (strcmp(argv[i], "-signet") == 0 || strcmp(argv[i], "-signet=1") == 0) {
                chainName = "signet";
                break;
            }
        }
