...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    // Check if this is an AuxPow block but we're not yet at the activation height
    if (block.IsAuxPow() && consensusParams.nAuxpowStartHeight >= 0) {
        // We can't check the height here, this will be checked in AcceptBlock
    }
